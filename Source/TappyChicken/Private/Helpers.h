...
class TAPPYCHICKEN_API HttpFileInput {
public:
  ...
  HttpFileInput &operator=(const FString &InFilePath) {
    SetFilePath(*InFilePath);
    return *this;
  }
  HttpFileInput &operator=(const TCHAR *InFilePath) {
    SetFilePath(InFilePath);
    return *this;
  }
...
// https://en.wikipedia.org/wiki/Base64#Variants_summary_table
template <typename T> FString Base64UrlEncode(const T &Value) {
...
template <typename T> inline FString ToUrlString(const T &Value) {
...
TAPPYCHICKEN_API bool ParseDateTime(const FString &DateTimeString,
