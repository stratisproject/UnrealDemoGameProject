...
struct TAPPYCHICKEN_API HttpRetryManager : public FManager,
...
struct TAPPYCHICKEN_API HttpRetryParams {
  HttpRetryParams(
...
class TAPPYCHICKEN_API Request {
public:
  virtual ~Request() {}
  ...
  void SetShouldRetry(const HttpRetryParams &Params = HttpRetryParams()) {
    RetryParams = Params;
  }
  const TOptional<HttpRetryParams> &GetRetryParams() const {
    return RetryParams;
  }
...
class TAPPYCHICKEN_API Response {
public:
  virtual ~Response() {}
  virtual bool FromJson(const TSharedPtr<FJsonValue> &JsonValue) = 0;
  ...
  void SetResponseString(const FString &InResponseString) {
    ResponseString = InResponseString;
  }
  const FString &GetResponseString() const { return ResponseString; }

  void SetHttpResponse(const FHttpResponsePtr &InHttpResponse) {
